 ...
 */
public class QueueRequestorImpl implements IBOBConstants {
	...
	public QueueRequestorImpl(String sendDestinationJndi, String receiveDestinationJndi, String targetService) {
		...
		BOBLogger.getLogger().log(Level.FINE, "In QueueRequestorImpl constructor using sendQueueName: " + 
	    ...
	    try {
			...
			BOBLogger.getLogger().log(Level.FINE, "In QueueRequestorImpl constructor JmsConfigJndi value is: " + 
					BOBPropertiesUtil.getJmsConfigJndi(), "QueueRequestorImpl", "Constructor");
			
			connectionFactory = (ConnectionFactory) context.lookup(BOBPropertiesUtil.getJmsConfigJndi());
			//connectionFactory = (ConnectionFactory) context.lookup("jms/MQCF");
			...
			BOBLogger.getLogger().log(Level.SEVERE, "Exception occurred in QueueRequestorImpl constructor: " + e.getMessage(), "QueueRequestorImpl", "Constructor");
		}
	}
	...
	public String sendAndReceive(String messageText) throws Exception {
		...
		try {
			...
			BOBLogger.getLogger().log(Level.INFO, "Second attempt at creating QueueSender", "QueueRequestorImpl", "sendAndReceive");
			createConnection();
			messageId = sendMessage(messageText);
			BOBLogger.getLogger().log(Level.INFO, "Second attempt at creating QueueSender is successful", "QueueRequestorImpl", "sendAndReceive");
		}

		BOBLogger.getLogger().log(Level.INFO, "Add document message sent successfully.  Corr ID: " + messageId, "QueueRequestorImpl", "sendAndReceive");
		...
		try {
			...
			e.printStackTrace();
			BOBLogger.getLogger().log(Level.SEVERE, "An Exception occurred receiving AddDocument response message: " + e.getMessage(), "QueueRequestorImpl", "sendAndReceive");
			throw e;
		} finally {
			receiver.close();		
			session.close();
		...
		if (receiveMessage == null){
			BOBLogger.getLogger().log(Level.SEVERE, "A null response was received from the AddDocument operation.  Possibly this request has timed out waiting for a reply. ", "QueueRequestorImpl", "sendAndReceive");
			return null;
		}
	...
	public String sendMessage(String messageText) throws JMSException, NamingException{
		...
		try {
			...
			BOBLogger.getLogger().log(Level.SEVERE, "An Exception occurred sendMessage : " + e.getMessage(), "QueueRequestorImpl", "sendMessage");
			throw e;
