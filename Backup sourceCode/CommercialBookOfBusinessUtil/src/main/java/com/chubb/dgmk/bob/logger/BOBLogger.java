package com.chubb.dgmk.bob.logger;
...
/**
 * @author P62240B
 * The BOBLogger is responsible for creating a Singleton instance for use
 * in logging all application level and exception messages. 
 */
public class BOBLogger implements IBOBConstants {

	// Single instance of class
	private static BOBLogger INSTANCE = new BOBLogger();
	...
	protected FileHandler createFileHandler() throws IOException {
		...
		FileHandler filehandler = new FileHandler(logLocationRoot() + logLocationSubfolder() + BOB_APPLICATION_LOG_PATH + BOB_APPLICATION_LOG_FILE, 2000000, 20);
		filehandler.setFormatter(new BOBLogMessageFormatter());
		return filehandler;
	}
	
	public static BOBLogger getLogger() {
		if (null == INSTANCE) {
			INSTANCE = new BOBLogger();
		}
		return INSTANCE;
	}
