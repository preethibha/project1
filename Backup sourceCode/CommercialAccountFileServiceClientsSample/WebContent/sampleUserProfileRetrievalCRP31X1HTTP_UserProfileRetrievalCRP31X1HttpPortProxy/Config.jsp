...
if (session.getAttribute("__default_endpoint__") == null) {
    session.setAttribute("__default_endpoint__", sampleUserProfileRetrievalCRP31X1HTTP_UserProfileRetrievalCRP31X1HttpPortProxyid._getDescriptor().getEndpoint());
}
if (submitAction != null && submitAction.equals("Update")) {
    sampleUserProfileRetrievalCRP31X1HTTP_UserProfileRetrievalCRP31X1HttpPortProxyid._getDescriptor().setEndpoint(request.getParameter("endpoint"));
}
else if (submitAction != null && submitAction.equals("Reset")) {
    sampleUserProfileRetrievalCRP31X1HTTP_UserProfileRetrievalCRP31X1HttpPortProxyid._getDescriptor().setEndpoint((String) session.getAttribute("__default_endpoint__"));
}
String endpoint = sampleUserProfileRetrievalCRP31X1HTTP_UserProfileRetrievalCRP31X1HttpPortProxyid._getDescriptor().getEndpoint();

boolean hasAsync = com.ibm.ccl.ws.jaxws.gstc.util.AsyncUtils.hasAsync(sampleUserProfileRetrievalCRP31X1HTTP_UserProfileRetrievalCRP31X1HttpPortProxyid.getClass());
